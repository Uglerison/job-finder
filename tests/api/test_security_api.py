from pathlib import Path

import pytest
from httpx import ASGITransport, AsyncClient

from job_finder.database import create_database_engine, create_session_factory, run_migrations
from job_finder.main import create_app
from job_finder.settings import Settings


@pytest.fixture
def application(tmp_path: Path):
    run_migrations(tmp_path)
    app = create_app(Settings(data_dir=tmp_path, environment="test"))
    app.state.session_factory = create_session_factory(create_database_engine(tmp_path))
    return app


@pytest.mark.anyio
async def test_csrf_token_is_cookie_bound_and_origin_is_loopback_only(application) -> None:
    transport = ASGITransport(app=application)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        token_response = await client.get("/api/security/csrf")
        token = token_response.json()["token"]
        missing_header = await client.post(
            "/api/jobs",
            headers={"Origin": "http://testserver"},
            json={
                "canonical_url": "https://jobs.example/security",
                "company": "Security Labs",
                "raw_content": "Descrição",
                "title": "Security Engineer",
            },
        )
        accepted = await client.post(
            "/api/jobs",
            headers={"Origin": "http://testserver", "X-CSRF-Token": token},
            json={
                "canonical_url": "https://jobs.example/security",
                "company": "Security Labs",
                "raw_content": "Descrição",
                "title": "Security Engineer",
            },
        )
        foreign = await client.post(
            "/api/jobs",
            headers={"Origin": "https://evil.example", "X-CSRF-Token": token},
            json={
                "canonical_url": "https://jobs.example/foreign",
                "company": "Foreign Labs",
                "raw_content": "Descrição",
                "title": "Security Engineer",
            },
        )

    assert token_response.status_code == 200
    assert token_response.cookies.get("job_finder_csrf") == token
    assert missing_header.status_code == 403
    assert accepted.status_code == 201
    assert foreign.status_code == 403
